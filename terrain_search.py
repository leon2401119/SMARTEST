...
def get_cmd_num(num,worker_num):
    cmd = ['gcc']
    if BASE_IS_OS:
        cmd += ['-Os']
    idx = len(FLAGS)-1
    ...
    file_list = glob.glob(os.path.join(CBENCH_PATH,TARGET,'src','*.c'))
    cmd.extend(file_list)
    cmd.extend(['-o',f'{TMP_DIR}/{worker_num}'])
...
def exhaustive_search():
    ...
    all_fitness = []
    while current < total:
        if current + WORKERS < total:
            works = WORKERS
        else:
            works = total - current + 1
        workload = [(current+i,i) for i in range(works)]
        all_fitness.extend(pool.starmap(get_fitness, workload))
